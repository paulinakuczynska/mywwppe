...
```
* Create a [Python virtual environment](https://flask.palletsprojects.com/en/2.1.x/installation/). This step is optional, but allows to keep dependencies in a separate place, so it ensures no conflicts with other Python projects on your computer```$ python3 -m venv venv```
...
```
* In the project folder set the FLASK_APP environment variable ```$ export FLASK_APP=run.py``` and run the application with ```$ flask run```. Due to that launching the application and its environment is necessary each time it is started, it is convenient to create an alias, e.g. ```FLASK_APP=run.py FLASK_ENV=development flask run```
## References
